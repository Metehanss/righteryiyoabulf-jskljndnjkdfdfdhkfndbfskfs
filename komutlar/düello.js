...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.reply("Oynamak istediğin kullanıcıyı etiketle!")
  
  if (opponent.bot) return message.reply('Malesef botlar ile düello yapamazsın.');
  if (opponent.id === message.author.id) return message.reply('Malesef kendin ile düello yapamazsın.');
		if (this.fighting.has(message.channel.id)) return message.reply('Kanal başına sadece bir düello meydana getirilebilir.');
		this.fighting.add(message.channel.id);
		try {
			if (!opponent.bot) {
                await message.channel.send(`${opponent}, düello yapma isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`);
